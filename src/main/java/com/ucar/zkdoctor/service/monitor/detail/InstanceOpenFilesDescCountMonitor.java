...
/**
 * Description: 实例打开文件描述符数量监控
 * Created on 2018/2/7 15:48
 *
 * @author 吕小玲(xiaoling.lv@ucarinc.com)
 */
public class InstanceOpenFilesDescCountMonitor extends MonitorBase {
            ...
            public void monitor(MonitorTask monitorTask, MonitorIndicator monitorIndicator) {
                try {
                    ...
                    for (InstanceState instanceState : instanceStateList) {
                        ...
                        if (instanceState.getOpenFileDescriptorCount() != null && monitorTimeIsLegal) {
                            monitorService.monitorInstanceState(instanceState, instanceState.getOpenFileDescriptorCount(), monitorTask,
                                    "打开文件描述符数量过多", monitorIndicator.getAlertValueUnit(), true);
                        }
                    }
                } catch (Exception e) {
    ...
    public void init(MonitorService monitorService, ClusterService clusterService,
        ...
        monitorIndicator.setIndicatorName("实例打开文件描述符数量监控");
        monitorIndicator.setClassName(this.getClass().getSimpleName());
        monitorIndicator.setDefaultAlertValue("5000");
