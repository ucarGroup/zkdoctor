 ...
 */
public class MachineCpuMonitor extends MonitorBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(MachineCpuMonitor.class);
            ...
            public void monitor(MonitorTask monitorTask, MonitorIndicator monitorIndicator) {
                try {
                    ...
                    for (InstanceInfo instanceInfo : instanceInfoList) {
                        ...
                        if (machineState != null && machineState.getCpuUsage() != null) {
                            monitorService.monitorMachineState(machineState, machineState.getCpuUsage(), monitorTask,
                                    "机器CPU使用率过高", monitorIndicator.getAlertValueUnit());
                        }
                    }
                } catch (Exception e) {
                    LOGGER.error("Thread monitor machine cpu error! clusterId is {}.", monitorTask.getClusterId(), e);
                } finally {
                    LOGGER.debug("Thread monitor machine cpu over. clusterId is {}.", monitorTask.getClusterId());
                }
            }
        });
    }
    ...
    public void init(MonitorService monitorService, ClusterService clusterService,
        ...
        monitorIndicator.setIndicatorName("机器CPU监控");
