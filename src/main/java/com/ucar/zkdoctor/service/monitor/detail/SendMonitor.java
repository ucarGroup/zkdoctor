 ...
 */
public class SendMonitor extends MonitorBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(SendMonitor.class);
                ...
                try {
                    if (currClusterStates.containsKey(monitorTask.getClusterId())) {
                        ...
                        if (clusterState != null) {
                            monitorService.monitorClusterState(clusterState, clusterState.getSentTotal(), monitorTask,
                                    "发包数过多", monitorIndicator.getAlertValueUnit());
                        }
                    }
                } catch (Exception e) {
    ...
    public void init(MonitorService monitorService, ClusterService clusterService,
        ...
        monitorIndicator.setIndicatorName("发包数监控");
