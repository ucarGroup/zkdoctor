...
/**
 * Description: 机器磁盘使用率监控（仅监控dataDir所在磁盘使用率）
 * Created on 2018/2/26 10:35
 *
 * @author 吕小玲(xiaoling.lv@ucarinc.com)
 */
public class MachineDiskMonitor extends MonitorBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(MachineDiskMonitor.class);
            ...
            public void monitor(MonitorTask monitorTask, MonitorIndicator monitorIndicator) {
                try {
                    ...
                    for (InstanceInfo instanceInfo : instanceInfoList) {
                        ...
                        if (machineState != null && machineState.getDiskUsage() != null) {
                            monitorService.monitorMachineState(machineState, machineState.getDiskUsage(), monitorTask,
                                    "机器磁盘使用率过高", monitorIndicator.getAlertValueUnit());
                        }
                    }
                } catch (Exception e) {
                    LOGGER.error("Thread monitor machine disk error! clusterId is {}.", monitorTask.getClusterId(), e);
                } finally {
                    LOGGER.debug("Thread monitor machine disk over. clusterId is {}.", monitorTask.getClusterId());
                }
            }
        });
    }
    ...
    public void init(MonitorService monitorService, ClusterService clusterService,
        ...
        monitorIndicator.setIndicatorName("机器磁盘使用率监控");
        ...
        monitorIndicator.setInfo("监控dataDir所在磁盘使用率");
